...
void FT5x06Touchscreen::setup() {
  ...
  // wait 1200ms to allow touch IC power rail to stabilise (FT3168 needs >1000ms on this board)
  this->set_timeout(1200, [this] { this->continue_setup_(); });
}
...
void FT5x06Touchscreen::update_touches() {
  ...
  if (!this->read_byte(FT5X06_TD_STATUS, &touch_cnt)) {
    ESP_LOGW(TAG, "Failed to read status");
    return;
  }
  if (touch_cnt == 0 || touch_cnt > MAX_TOUCHES)
    return;
